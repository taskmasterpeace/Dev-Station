...
async function showStatus(id) {
  if (id) {
    ...
    for (const p of res.data) {
      ...
      console.log(`  ${formatStatus(p.status)} ${p.id.padEnd(14)} ${p.status.padEnd(10)} ${port.padEnd(6)}  ${p.name}${uptime}`);
    }
    console.log('');
  }
}
...
Commands:
  ...
  list            List all project IDs (useful for scripting)
